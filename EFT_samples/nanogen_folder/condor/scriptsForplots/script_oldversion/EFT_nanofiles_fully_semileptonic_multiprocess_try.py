...
h_jet_multiplicity_from_top = ROOT.TH1F('h_jet_multiplicity_from_top', 'Jet Multiplicity From Top;Number of Jets;Events', 10, 0, 10)
...
def process_event(entry, histograms, relevant_pdgIds):
    ...
    if not b_jet_matched:
        ...
        # if leading_b_quark is not None:
            # print("leading_b_quark[0].Pt():", leading_b_quark[0].Pt())
        # else:
            # print("No leading b-quark found")
    ...
    if len(w_quarks) == 2:
        ...
        for b_quark in b_quarks:
            ...
            if (channel == "electron") and passed_jet_cut:
                # print("b_quark pt:", b_vector.Pt() )
                # if b_vector.Pt() <30:
                    # print("found b_vector.Pt() <30 ")
        ...
        if passed_jet_cut:
            jets_from_top_count = 0
            
            for i in range(entry.nGenJet):
                jet = ROOT.TLorentzVector()
                jet.SetPtEtaPhiM(entry.GenJet_pt[i], entry.GenJet_eta[i], entry.GenJet_phi[i], 0) 
                
                # Match jets to W-boson and b-quark decay products
                for w_4vec in w_bosons:
                    if deltaR(jet.Eta(), jet.Phi(), w_4vec.Eta(), w_4vec.Phi()) < 0.4:
                        jets_from_top_count += 1
                        break  
                
                for b_quark in b_quarks:
                    b_vector, _ = b_quark
                    if deltaR(jet.Eta(), jet.Phi(), b_vector.Eta(), b_vector.Phi()) < 0.4:
                        jets_from_top_count += 1
                        break 
                    
            histograms['h_jet_multiplicity_from_top'].Fill(jets_from_top_count)
            ...
            print("w_decay_quark_jets: ", len(w_decay_quark_jets))
            print("b_quark_jets: ", len(b_quark_jets))
            print("combined: ", len(combined_jets))
...
def analyze(filename):
    ...
    "h_jet_multiplicity_from_top" : h_jet_multiplicity_from_top
